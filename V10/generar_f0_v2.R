...
#     sin Concentracion, template v2. v2.1: + Contribucion YTD (v9).
# ============================================================
...
# --- Performance vs target (v9.8) ---
VC_DESDE     <- "2024-01-01"   # desde cuando cargar la serie (hoja ValorCuota del xlsm)
TARGET_ANUAL <- 0.045                                          # tasa objetivo anual (definir con Guillermo)
# ------------------------------------------------------------
...
while (ncol(hist_raw) < 34) hist_raw[[ncol(hist_raw) + 1]] <- NA
...
for (arch in archivos) {
  ...
  # --- Contribucion YTD (v9, fila 4+): 10 cols; hoja puede no existir en copias viejas ---
  cy_raw <- leer_hoja(arch, "Contribucion YTD", 4)
  cy <- list()
  if (!is.null(cy_raw) && nrow(cy_raw) > 0) {
    for (i in seq_len(nrow(cy_raw))) {
      if (is.na(cy_raw[i, 2]) || cy_raw[i, 2] == "") next
      cy[[length(cy) + 1]] <- list(
        as.character(cy_raw[i, 1]), as.character(cy_raw[i, 2]),
        as.character(cy_raw[i, 3]), num0(cy_raw[i, 4]),
        num0(cy_raw[i, 5]), num0(cy_raw[i, 6]),
        num0(cy_raw[i, 7]), num0(cy_raw[i, 8]),
        num0(cy_raw[i, 9]), num0(cy_raw[i, 10]),
        num0(cy_raw[i, 11]), as.character(cy_raw[i, 12]))
    }
  }

  # --- Posicionamiento vs Benchmark (v9.2, fila 3+): concepto, actual, bmk, gap ---
  pos_raw <- leer_hoja(arch, "Posicionamiento", 3)
  pos <- list()
  if (!is.null(pos_raw) && nrow(pos_raw) > 0) {
    for (i in seq_len(nrow(pos_raw))) {
      if (is.na(pos_raw[i, 1]) || pos_raw[i, 1] == "") next
      pos[[length(pos) + 1]] <- list(
        as.character(pos_raw[i, 1]), num0(pos_raw[i, 2]),
        num0(pos_raw[i, 3]), num0(pos_raw[i, 4]))
    }
  }
  ...
  det_json_partes <- c(det_json_partes, paste0(
    ...
    '"contribYTD":', lista_filas_json(cy), ',',
    '"pos":', lista_filas_json(pos), ',',
    '"venc":',   lista_filas_json(ven), '}'))
}

# ----------------- 2a) VALOR CUOTA F0 (archivo del equipo) --
vc <- list()
vc_raw <- tryCatch(read.xlsx(RUTA_XLSM, sheet = "ValorCuota", colNames = FALSE,
                             startRow = 2, detectDates = TRUE, skipEmptyRows = TRUE),
                   error = function(e) NULL)
if (!is.null(vc_raw) && nrow(vc_raw) > 0) {
  for (i in seq_len(nrow(vc_raw))) {
    f <- fecha_iso(vc_raw[i, 1])
    v <- num0(vc_raw[i, 2])          # col B = VC F0 (copiado por la macro)
    if (is.na(f) || is.na(v) || v <= 0) next
    if (f >= VC_DESDE) vc[[length(vc) + 1]] <- list(f, v)
  }
  if (length(vc) > 1) {
    ordv <- order(vapply(vc, function(x) x[[1]], ""))
    vc <- vc[ordv]
  }
  cat("  Valor cuota:", length(vc), "observaciones\n")
} else {
  cat("  Valor cuota: archivo no disponible (el dashboard omitira Performance)\n")
}

# ----------------- 2b) PROFORMA (hoja manual del xlsm) ------
prof <- list()
prof_raw <- tryCatch(read.xlsx(RUTA_XLSM, sheet = "Proforma", colNames = FALSE,
                               startRow = 3, skipEmptyRows = TRUE), error = function(e) NULL)
if (!is.null(prof_raw) && nrow(prof_raw) > 0) {
  for (i in seq_len(nrow(prof_raw))) {
    if (is.na(prof_raw[i, 1]) || prof_raw[i, 1] == "") next
    prof[[length(prof) + 1]] <- list(as.character(prof_raw[i, 1]), num0(prof_raw[i, 2]))
  }
}
...
json <- paste0(
  ...
  '"proforma":', lista_filas_json(prof), ',',
  '"vc":', lista_filas_json(vc), ',',
  '"targetAnual":', val_json(TARGET_ANUAL), ',',
