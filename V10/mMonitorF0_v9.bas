...
'============================================================
' MONITOR FONDO 0 - Modulo unico (v8)  [v9.9: copia automatica del valor cuota a hoja ValorCuota (Config B11)]
...
Private Const SH_CYTD  As String = "Contribucion YTD"
Private Const SH_POS   As String = "Posicionamiento"
Private Const SH_BMK   As String = "Benchmark"
...
Private Const C_PRE = 16     ' precio actual (vector col K)
Private Const C_PREANT = 17  ' precio observacion anterior
Private Const C_NCOLS = 17

Private mTC As Double        ' TC USD/PEN leido del FMS del dia (0 = no encontrado)
...
Public Sub CrearEsqueleto()
    ...
    With ws
        ...
        .Range("A8").Value = "COL CRUCE FMS (4=D codigo SBS)"
        ...
        If Len(Cfg("B8")) = 0 Then .Range("B8").Value = 4
    ...
    Hoja(SH_CON): Hoja(SH_VEN): Hoja(SH_CTR): Hoja(SH_CYTD): Hoja(SH_POS)
    PrepararBenchmark
    PrepararProforma
...
Private Sub EncabezadoHistorico(ws As Worksheet)
    ws.Range("A1:AH1").Value = Array("FECHA", "DIAS", "VALCART MM", _
        "YTM FONDO", "YTM NEGOC", "YTM BONOS", "YTM PC", "YTM DEP", _
        "SPR TOT", "SPR BONOS", "SPR PC", _
        "DUR TOT", "DUR BONOS", "DUR PC", "DUR DEP", _
        "COBERT RTG", "RATING LP", "HHI", "TOP5", "TOP10", _
        "VENC 30D", "VENC 90D", "CARRY DIA", "CARRY MTD", "CARRY YTD", _
        "N INSTR", "N SIN MATCH", "SCORE LP", "SCORE CP", "TC", _
        "PESO DEP", "PESO STDI", "PESO SOB", "PESO CORP")
    FormatoInstitucional ws.Range("A1:AH1")
End Sub
...
Private Function CargarDepositos() As Object
    ...
    uf = UltFila(ws)
    For i = 2 To uf
        cod = NormCod(ws.Cells(i, 3).Value)              ' C = codigo SBS
...
'============================================================
' RATING - dos escalas peruanas separadas (CP y LP)
' CP (corto): identificado por prefijo CLP/CP en el texto.
'   CLP 1+ = 10 | CLP 1 = 7 | CLP 1- = 4
' LP (largo): local anclada al soberano.
'   Gobierno Central = 10 | LPI BB+ (= AAA local) = 7 |
'   LP AAA = 7 | LP AA+ / LP AA = 5 | vecinos mapeados abajo.
' Sin mapeo -> -1 (se reporta, nunca se promedia en silencio).
'============================================================
Private Function NormRating(ByVal r As String) As String
    NormRating = UCase$(Replace(Trim$(r), " ", ""))
End Function

Public Function TipoRating(ByVal r As String) As String
    Dim s As String
    s = NormRating(r)
    If Left$(s, 3) = "CLP" Or Left$(s, 2) = "CP" Then
        TipoRating = "CP"
    Else
        TipoRating = "LP"
    End If
...
Public Function PuntajeRating(ByVal r As String) As Double
    Select Case NormRating(r)
        ...
        Case "LPIBBB-", "LPIBBB": PuntajeRating = 10
        Case "LPIBB+": PuntajeRating = 7
        Case "LPIBB":  PuntajeRating = 5
        Case "LPIBB-": PuntajeRating = 4
        Case "LPIB+":  PuntajeRating = 3.5
        Case "LPIB":   PuntajeRating = 3
        ' --- largo plazo: escala local (LP) ---
        Case "LPAAA":  PuntajeRating = 7
        Case "LPAA+", "LPAA": PuntajeRating = 5
        Case "LPAA-": PuntajeRating = 4
        Case "LPA+":  PuntajeRating = 3.5
        Case "LPA", "LPA-": PuntajeRating = 3
        Case "LPBB+": PuntajeRating = 3
        Case Else: PuntajeRating = -1      ' SIN MAPEO -> reportar
    End Select
End Function

Private Function ScoreALetraLP(ByVal sc As Double) As String
    If sc <= 0 Then ScoreALetraLP = "-": Exit Function
    Select Case sc
        Case Is >= 8.5: ScoreALetraLP = "Gob/Sob"
        Case Is >= 6:   ScoreALetraLP = "AAA"
        Case Is >= 4.5: ScoreALetraLP = "AA+/AA"
        Case Is >= 3.5: ScoreALetraLP = "AA-"
        Case Is >= 2.5: ScoreALetraLP = "A/BB+"
        Case Else: ScoreALetraLP = "<A"
    End Select
End Function
...
Public Function CalcularDia(ByVal fechaVector As Date, ByVal escribirHojas As Boolean) As Boolean
    ...
    ' --- 1b) Vector ANTERIOR -> precios para variacion ---
    Dim dPre As Object, fVAnt As Long
    Set dPre = CreateObject("Scripting.Dictionary")
    fVAnt = VectorAnterior(CLng(fechaVector), dV)
    If fVAnt > 0 Then
        Set wbV = Workbooks.Open(dV(fVAnt), ReadOnly:=True, UpdateLinks:=0)
        With wbV.Worksheets(1)
            uf = UltFila(wbV.Worksheets(1))
            For i = 2 To uf
                cod = NormCod(.Cells(i, 1).Value)
                If Len(cod) > 0 Then
                    If Not dPre.Exists(cod) Then dPre.Add cod, Val(.Cells(i, 11).Value)
                End If
            Next i
        End With
        wbV.Close SaveChanges:=False: Set wbV = Nothing
    ...
    For i = 2 To uf
        If NormFond(wsF.Cells(i, 1).Value) = codFondo Then
            nD = nD + 1
            cod = NormCod(wsF.Cells(i, colCruce).Value)
            ...
            Dim vF As Variant
            vF = ToFecha(wsF.Cells(i, 12).Value)
            If IsDate(vF) Then
                cart(nD, C_VCTO) = CDate(vF)
                cart(nD, C_DIAS) = CDbl(CDate(vF) - fechaVector)
            Else
            ...
            If dVec.Exists(cod) Then
                ...
                cart(nD, C_PRE) = v(3)
                If dPre.Exists(cod) Then cart(nD, C_PREANT) = dPre(cod) Else cart(nD, C_PREANT) = 0
                cart(nD, C_VEC) = 1
    ...
    If escribirHojas Then EscribirContribYTD fechaVector, dV, cart, nD
    If escribirHojas Then EscribirPosicionamiento fechaVector, cart, nD
    CalcularDia = True
    Exit Function
...
Private Sub ProcesarDia(ByVal fObs As Date, ByVal fFMS As Date, _
    ...
    Dim mtoLP As Double, scLP As Double, mtoCP As Double, scCP As Double
    Dim mtoSinMapeo As Double, pj As Double
    ...
    For i = 1 To nD
        ...
        Else
            ...
            Else
                nSin = nSin + 1
            End If
            ' rating solo universo negociable, escalas CP y LP separadas
            If Len(CStr(cart(i, C_RAT))) > 0 Then
                pj = PuntajeRating(CStr(cart(i, C_RAT)))
                If pj >= 0 Then
                    If TipoRating(CStr(cart(i, C_RAT))) = "CP" Then
                        mtoCP = mtoCP + cart(i, C_MTO): scCP = scCP + cart(i, C_MTO) * pj
                    Else
                        mtoLP = mtoLP + cart(i, C_MTO): scLP = scLP + cart(i, C_MTO) * pj
                    End If
                Else
                    mtoSinMapeo = mtoSinMapeo + cart(i, C_MTO)
                End If
            End If
    ...
    If mtoLP > 0 Then scLP = scLP / mtoLP
    If mtoCP > 0 Then scCP = scCP / mtoCP
    ...
    ' composicion por grupo para la pestana Summary (toda la cartera)
    Dim mDep2 As Double, mSTDI As Double, mSob As Double, mCorp As Double
    For i = 1 To nD
        Select Case CStr(cart(i, C_CAT))
            Case "Depositos a Plazo": mDep2 = mDep2 + cart(i, C_MTO)
            Case "Papeles Comerciales": mSTDI = mSTDI + cart(i, C_MTO)
            Case Else
                If UCase$(Replace(CStr(cart(i, C_RAT)), " ", "")) = "GOBIERNOCENTRAL" Then
                    mSob = mSob + cart(i, C_MTO)
                Else
                    mCorp = mCorp + cart(i, C_MTO)
                End If
        End Select
    Next i
    ...
    EscribirHistorico fObs, diasTr, mtoTot, ytmF, ytmN, ytmB, ytmP, ytmD, _
                      ...
                      IIf(mtoNeg > 0, (mtoLP + mtoCP) / mtoNeg, 0), ScoreALetraLP(scLP), _
                      hhi, t5, t10, v30, v90, carryDia, nD, nSin, scLP, scCP, IIf(mTC > 0, mTC, CfgNum("B10", 0)), _
                      IIf(mtoTot > 0, mDep2 / mtoTot, 0), IIf(mtoTot > 0, mSTDI / mtoTot, 0), _
                      IIf(mtoTot > 0, mSob / mtoTot, 0), IIf(mtoTot > 0, mCorp / mtoTot, 0)
...
Private Sub EscribirHistorico(ByVal fObs As Date, ByVal diasTr As Double, _
    ...
    ByVal nInstr As Long, ByVal nSin As Long, _
    ByVal scLP As Double, ByVal scCP As Double, ByVal tc As Double, _
    ByVal pDep As Double, ByVal pSTDI As Double, ByVal pSob As Double, ByVal pCorp As Double)
    ...
    ws.Cells(fila, 28).Value = scLP
    ws.Cells(fila, 29).Value = scCP
    If tc > 0 Then ws.Cells(fila, 30).Value = tc
    ws.Cells(fila, 31).Value = pDep
    ws.Cells(fila, 32).Value = pSTDI
    ws.Cells(fila, 33).Value = pSob
    ws.Cells(fila, 34).Value = pCorp

    ' ordenar por fecha y recalcular carry acumulado MTD/YTD de toda la hoja
    uf = UltFila(ws)
    If uf > 2 Then
        ws.Range("A2:AH" & uf).Sort Key1:=ws.Range("A2"), Order1:=xlAscending, Header:=xlNo
    End If
    ...
    ws.Range("AB2:AC" & uf).NumberFormat = "0.00"
    ws.Range("AD2:AD" & uf).NumberFormat = "0.0000"
    ws.Range("AE2:AH" & uf).NumberFormat = "0.00%"
End Sub
...
Private Sub EscribirCartera(ByVal fObs As Date, ByVal fFMS As Date, _
    ...
    Set ws = Hoja(SH_CART)
    ws.Cells.Clear
    ws.Range("A1:S1").Value = Array("FECHA OBS", "FECHA FMS", "CODIGO", "EMISOR", "ASSET CLASS", _
        "CATEGORIA", "MON", "CANTIDAD", "MONTO MM", "RATING", "VCTO", "DIAS", _
        "YTW", "SPREAD PBS", "DURACION", "EN VECTOR", "DEPOSITO", "PESO", "DTS")
    For i = 1 To nD
        ...
        ws.Cells(i + 1, 19).Value = cart(i, C_DUR) * cart(i, C_SPR)
    Next i
    fila = nD + 1
    ...
    ws.Range("S2:S" & fila).NumberFormat = "#,##0.0"
    FormatoInstitucional ws.Range("A1:S" & fila)
...
Private Sub EscribirMetricas(ByVal fObs As Date, ByRef cart() As Variant, ByVal nD As Long, _
    ...
    ' ---------- CALIDAD: dos escalas separadas LP / CP ----------
    Dim dLP As Object, dCP As Object, mLPb As Double, mCPb As Double
    Set dLP = CreateObject("Scripting.Dictionary")
    Set dCP = CreateObject("Scripting.Dictionary")
    For i = 1 To nD
        If cart(i, C_DEP) = 0 And Len(CStr(cart(i, C_RAT))) > 0 Then
            k = cart(i, C_RAT)
            If TipoRating(CStr(k)) = "CP" Then
                If dCP.Exists(k) Then dCP(k) = dCP(k) + cart(i, C_MTO) Else dCP.Add k, cart(i, C_MTO)
                mCPb = mCPb + cart(i, C_MTO)
            Else
                If dLP.Exists(k) Then dLP(k) = dLP(k) + cart(i, C_MTO) Else dLP.Add k, cart(i, C_MTO)
                mLPb = mLPb + cart(i, C_MTO)
            End If
        End If
    Next i
    Set ws = Hoja(SH_CAL)
    ...
    ws.Range("A3").Value = "LARGO PLAZO": ws.Range("A3").Font.Bold = True
    ws.Range("A4:D4").Value = Array("RATING", "MONTO MM", "PESO GRUPO", "PUNTAJE")
    fila = 5
    For Each k In dLP.Keys
        ws.Cells(fila, 1).Value = k
        ws.Cells(fila, 2).Value = dLP(k)
        If mLPb > 0 Then ws.Cells(fila, 3).Value = dLP(k) / mLPb
        ws.Cells(fila, 4).Value = PuntajeRating(CStr(k))
        If PuntajeRating(CStr(k)) < 0 Then ws.Cells(fila, 4).Value = "SIN MAPEO"
        fila = fila + 1
    Next k
    If fila > 5 Then
        ws.Range("A5:D" & fila - 1).Sort Key1:=ws.Range("B5"), Order1:=xlDescending, Header:=xlNo
        FormatoInstitucional ws.Range("A4:D" & fila - 1)
    End If

    Dim fCP As Long
    fCP = fila + 2
    ws.Cells(fCP, 1).Value = "CORTO PLAZO": ws.Cells(fCP, 1).Font.Bold = True
    ws.Range(ws.Cells(fCP + 1, 1), ws.Cells(fCP + 1, 4)).Value = Array("RATING", "MONTO MM", "PESO GRUPO", "PUNTAJE")
    fila = fCP + 2
    For Each k In dCP.Keys
        ws.Cells(fila, 1).Value = k
        ws.Cells(fila, 2).Value = dCP(k)
        If mCPb > 0 Then ws.Cells(fila, 3).Value = dCP(k) / mCPb
        ws.Cells(fila, 4).Value = PuntajeRating(CStr(k))
        fila = fila + 1
    Next k
    If fila > fCP + 2 Then
        ws.Range(ws.Cells(fCP + 2, 1), ws.Cells(fila - 1, 4)).Sort _
            Key1:=ws.Cells(fCP + 2, 2), Order1:=xlDescending, Header:=xlNo
        FormatoInstitucional ws.Range(ws.Cells(fCP + 1, 1), ws.Cells(fila - 1, 4))
    End If
    ws.Range("B5:B" & fila).NumberFormat = "#,##0.00"
    ws.Range("C5:C" & fila).NumberFormat = "0.00%"
    ws.Columns("A:D").AutoFit

    ' ---------- CONCENTRACIONp emisores (negociable) ----------
    Set ws = Hoja(SH_CON)
    ws.Cells.Clear
    ...
    ' ---------- CONTRIBUCION: variacion de precio (vector col K) + deps agregado ----------
    Set ws = Hoja(SH_CTR)
    ws.Cells.Clear
    ws.Range("A1").Value = "CONTRIBUCION (" & diasTr & " dias) — " & Format(fObs, "dd/mm/yyyy")
    ws.Range("A1").Font.Bold = True
    ws.Range("A3:I3").Value = Array("CODIGO", "EMISOR", "CATEGORIA", "PESO FONDO", _
                                    "P ANT", "P ACT", "VAR %", "CONTRIB %", "METODO")
    fila = 4
    Dim base As Double, ctrDep As Double, pesoDep As Double
    Dim wI As Double, rI As Double, totCtr As Double
    base = CfgNum("B6", 365)
    For i = 1 To nD
        If mtoTot > 0 Then
            wI = cart(i, C_MTO) / mtoTot
            If cart(i, C_DEP) = 1 Then
                ' depositos: carry con tasa pactada, agregado
                If cart(i, C_YTW) > 0 Then
                    ctrDep = ctrDep + wI * (cart(i, C_YTW) / base * diasTr) / 100
                End If
                pesoDep = pesoDep + wI
            ElseIf cart(i, C_VEC) = 1 Then
                If cart(i, C_PREANT) > 0 And cart(i, C_PRE) > 0 Then
                    ' retorno real: variacion de precio del vector
                    rI = cart(i, C_PRE) / cart(i, C_PREANT) - 1
                    ws.Cells(fila, 5).Value = cart(i, C_PREANT)
                    ws.Cells(fila, 6).Value = cart(i, C_PRE)
                    ws.Cells(fila, 9).Value = "Precio"
                Else
                    ' sin precio anterior: respaldo por carry
                    rI = (cart(i, C_YTW) / base * diasTr) / 100
                    ws.Cells(fila, 9).Value = "Carry (sin precio ant.)"
                End If
                ws.Cells(fila, 1).Value = cart(i, C_COD)
                ws.Cells(fila, 2).Value = cart(i, C_EMI)
                ws.Cells(fila, 3).Value = cart(i, C_CAT)
                ws.Cells(fila, 4).Value = wI
                ws.Cells(fila, 7).Value = rI
                ws.Cells(fila, 8).Value = wI * rI
                totCtr = totCtr + wI * rI
                fila = fila + 1
            End If
        End If
    ...
    ws.Cells(fila, 8).Value = ctrDep
    ws.Cells(fila, 9).Value = "Carry (tasa pactada)"
    totCtr = totCtr + ctrDep
    fila = fila + 1
    If fila > 5 Then
        ws.Range("A4:I" & fila - 1).Sort Key1:=ws.Range("H4"), Order1:=xlDescending, Header:=xlNo
    End If
    ws.Cells(fila, 2).Value = "TOTAL"
    ws.Cells(fila, 2).Font.Bold = True
    ws.Cells(fila, 8).Value = totCtr
    ws.Cells(fila, 8).Font.Bold = True
    FormatoInstitucional ws.Range("A3:I" & fila)
    ws.Range("D4:D" & fila).NumberFormat = "0.00%"
    ws.Range("E4:F" & fila).NumberFormat = "#,##0.0000"
    ws.Range("G4:H" & fila).NumberFormat = "0.0000%"
...
Public Sub CorrerDiario()
    ...
    If CalcularDia(CDate(o(UBound(o))), True) Then
        Dim msgCopia As String, msgVC As String
        msgVC = ActualizarValorCuota()
        msgCopia = GuardarCopiaDia(CDate(o(UBound(o))))
        PrepararApp False
        MsgBox "Dia calculado: " & Format(CDate(o(UBound(o))), "dd/mm/yyyy") & vbCrLf & _
               msgCopia & vbCrLf & msgVC & vbCrLf & _
...
'============================================================
' COPIA DIARIA: exporta las hojas del dia a
' F0_Monitor_AAAAMMDD.xlsx en la carpeta de Config B3.
' Devuelve el mensaje para el cartel final.
'============================================================
Private Function GuardarCopiaDia(ByVal fObs As Date) As String
    Dim carpeta As String, ruta As String
    Dim wbOut As Workbook

    carpeta = ConBarra(Cfg("B3"))
    If Len(carpeta) = 0 Then
        GuardarCopiaDia = "Sin carpeta en Config B3: no se guardo copia del dia."
        Exit Function
    End If
    If Len(Dir(carpeta, vbDirectory)) = 0 Then
        GuardarCopiaDia = "Carpeta de Config B3 no existe: no se guardo copia."
        Exit Function
    End If

    ruta = carpeta & "F0_Monitor_" & Format(fObs, "yyyymmdd") & ".xlsx"
    On Error GoTo Fallo
    If Len(Dir(ruta)) > 0 Then Kill ruta

    ThisWorkbook.Sheets(Array(SH_CART, SH_YLD, SH_CAL, SH_CON, SH_VEN, SH_CTR, SH_CYTD, SH_POS)).Copy
    Set wbOut = ActiveWorkbook
    wbOut.SaveAs Filename:=ruta, FileFormat:=51
    wbOut.Close SaveChanges:=False

    GuardarCopiaDia = "Copia del dia: " & ruta
    Exit Function

Fallo:
    On Error Resume Next
    If Not wbOut Is Nothing Then wbOut.Close SaveChanges:=False
    GuardarCopiaDia = "No se pudo guardar la copia (" & Err.Description & ")."
End Function
...
Public Sub CorrerBackfill()
    ...
    If MsgBox("Backfill de " & (UBound(o) - LBound(o) + 1) & " observaciones" & vbCrLf & _
              ...
              "Guarda avance cada 25. ¿Continuar?", vbYesNo + vbQuestion) = vbNo Then Exit Sub
    ...
    For i = LBound(o) To UBound(o)
        ...
        Else
            ...
            If nHech > 0 And nHech Mod 25 = 0 Then
                Application.StatusBar = "Guardando avance... (" & nHech & " calculadas)"
                ThisWorkbook.Save
            End If
        End If
    Next i
...
'============================================================
' VECTOR ANTERIOR: fecha del vector inmediatamente previo
'============================================================
Private Function VectorAnterior(ByVal fV As Long, dV As Object) As Long
    Dim k As Variant, mejor As Long
    mejor = 0
    For Each k In dV.Keys
        If CLng(k) < fV And CLng(k) > mejor Then mejor = CLng(k)
    Next k
    VectorAnterior = mejor
End Function

'============================================================
' NORMALIZACION DE CODIGOS PARA EL CRUCE
' Sin guiones ni espacios, en mayusculas. Se aplica por igual
' al vector, al FMS y a la hoja Depositos.
'============================================================
Private Function NormCod(ByVal v As Variant) As String
    Dim s As String
...
' Convierte vencimientos AAAAMMDD (numero o texto) a fecha real.
' Igual que ToFecha de Duracion.xlsm; acepta tambien fechas ya validas.
Private Function ToFecha(ByVal v As Variant) As Variant
    Dim s As String
    If IsDate(v) Then ToFecha = CDate(v): Exit Function
    If IsNumeric(v) Then s = Format$(v, "0") Else s = Trim$(CStr(v))
    If Len(s) = 8 And IsNumeric(s) Then
        On Error Resume Next
        ToFecha = DateSerial(CLng(Left$(s, 4)), CLng(Mid$(s, 5, 2)), CLng(Right$(s, 2)))
        On Error GoTo 0
        If Not IsDate(ToFecha) Then ToFecha = ""
    Else
        ToFecha = ""
    End If
End Function
...
'============================================================
' CONTRIBUCION YTD (v9): variacion de precio vs vector del
' cierre de anio anterior. Aproximacion: peso ACTUAL x var.
' Instrumentos nuevos: primer vector disponible de cada mes.
' Depositos: carry aproximado (tasa actual x dias transcurridos).
'============================================================
Private Function VectorRefAnio(ByVal fObs As Date, ByRef dV As Object) As Long
    Dim objetivo As Long, arrV() As Long, j As Long
    objetivo = CLng(DateSerial(Year(fObs) - 1, 12, 31))
    If dV.Count = 0 Then Exit Function
    arrV = ClavesOrdenadas(dV)
    For j = LBound(arrV) To UBound(arrV)
        If arrV(j) <= objetivo Then VectorRefAnio = arrV(j)
        If arrV(j) > objetivo Then Exit For
    Next j
End Function

Private Function PreciosDeVector(ByVal ruta As String) As Object
    Dim d As Object, wb As Workbook, uf As Long, i As Long, cod As String
    Set d = CreateObject("Scripting.Dictionary")
    On Error GoTo Fin
    Set wb = Workbooks.Open(ruta, ReadOnly:=True, UpdateLinks:=0)
    uf = UltFila(wb.Worksheets(1))
    With wb.Worksheets(1)
        For i = 2 To uf
            cod = NormCod(.Cells(i, 1).Value)
            If Len(cod) > 0 Then
                If Not d.Exists(cod) Then d.Add cod, Val(.Cells(i, 11).Value)
            End If
        Next i
    End With
Fin:
    ...
    Set PreciosDeVector = d
End Function

Private Sub EscribirContribYTD(ByVal fObs As Date, ByRef dV As Object, _
                               ByRef cart() As Variant, ByVal nD As Long)
    Dim ws As Worksheet, i As Long, fila As Long, j As Long
    Dim fRefA As Long, fRefT As Long
    Dim dRefA As Object, dRefT As Object, dRefF As Object
    Dim mtoTot As Double, wI As Double, totY As Double, totQ As Double
    Dim depY As Double, depQ As Double, pesoDep As Double, base As Double
    Dim cod As String, pA As Double, pT As Double, rY As Double, rQ As Double

    Set ws = Hoja(SH_CYTD)
    ws.Cells.Clear

    fRefA = VectorRefAnio(fObs, dV)
    fRefT = VectorRefTrim(fObs, dV)
    If fRefA = 0 Then
        ws.Range("A1").Value = "CONTRIBUCION YTD - sin vector de cierre de anio en las carpetas"
        Exit Sub
    End If

    For i = 1 To nD: mtoTot = mtoTot + cart(i, C_MTO): Next i
    If mtoTot = 0 Then Exit Sub

    Set dRefA = PreciosDeVector(dV(fRefA))
    If fRefT > fRefA Then
        Set dRefT = PreciosDeVector(dV(fRefT))
    Else
        Set dRefT = dRefA          ' inicio de anio: QTD = YTD
        fRefT = fRefA
    End If

    ' fecha de referencia YTD usada por codigo (fallback para nuevos)
    Set dRefF = CreateObject("Scripting.Dictionary")
    For i = 1 To nD
        cod = CStr(cart(i, C_COD))
        If dRefA.Exists(cod) Then
            If dRefA(cod) > 0 And Not dRefF.Exists(cod) Then dRefF.Add cod, fRefA
        End If
    Next i

    Dim faltan As Object, arrV() As Long, mesVisto As Object, dTmp As Object, kk As Variant, mk As String
    Set faltan = CreateObject("Scripting.Dictionary")
    For i = 1 To nD
        cod = CStr(cart(i, C_COD))
        If cart(i, C_DEP) = 0 And cart(i, C_VEC) = 1 And Not dRefF.Exists(cod) Then
            If Not faltan.Exists(cod) Then faltan.Add cod, 1
        End If
    Next i
    If faltan.Count > 0 Then
        arrV = ClavesOrdenadas(dV)
        Set mesVisto = CreateObject("Scripting.Dictionary")
        For j = LBound(arrV) To UBound(arrV)
            If faltan.Count = 0 Then Exit For
            If arrV(j) > fRefA And arrV(j) < CLng(fObs) Then
                mk = Format(CDate(arrV(j)), "yyyymm")
                If Not mesVisto.Exists(mk) Then
                    mesVisto.Add mk, 1
                    Application.StatusBar = "Contrib YTD: buscando precios base " & Format(CDate(arrV(j)), "mm/yyyy")
                    Set dTmp = PreciosDeVector(dV(arrV(j)))
                    For Each kk In faltan.Keys
                        If dTmp.Exists(CStr(kk)) Then
                            If dTmp(CStr(kk)) > 0 Then
                                dRefA(CStr(kk)) = dTmp(CStr(kk))
                                dRefF(CStr(kk)) = arrV(j)
                                faltan.Remove kk
                            End If
                        End If
                    Next kk
    ...
    ws.Range("A1").Value = "CONTRIBUCION YTD (desde " & Format(CDate(fRefA), "dd/mm/yyyy") & _
        ") y QTD (desde " & Format(CDate(fRefT), "dd/mm/yyyy") & ") — " & Format(fObs, "dd/mm/yyyy") & _
        "  (aprox: peso actual x variacion de precio)"
    ws.Range("A1").Font.Bold = True
    ws.Range("A3:L3").Value = Array("CODIGO", "EMISOR", "CATEGORIA", "PESO FONDO", "P ACT", _
        "P REF ANIO", "VAR % YTD", "CONTRIB % YTD", _
        "P REF TRIM", "VAR % QTD", "CONTRIB % QTD", "METODO")
    fila = 4
    base = CfgNum("B6", 365)
    For i = 1 To nD
        wI = cart(i, C_MTO) / mtoTot
        ...
        If cart(i, C_DEP) = 1 Then
            If cart(i, C_YTW) > 0 Then
                depY = depY + wI * (cart(i, C_YTW) / base * CDbl(CLng(fObs) - fRefA)) / 100
                depQ = depQ + wI * (cart(i, C_YTW) / base * CDbl(CLng(fObs) - fRefT)) / 100
            End If
            pesoDep = pesoDep + wI
        ElseIf cart(i, C_VEC) = 1 And dRefF.Exists(cod) Then
            pA = dRefA(cod)
            If pA > 0 And cart(i, C_PRE) > 0 Then
                rY = cart(i, C_PRE) / pA - 1
                ws.Cells(fila, 1).Value = cod
                ws.Cells(fila, 2).Value = cart(i, C_EMI)
                ws.Cells(fila, 3).Value = cart(i, C_CAT)
                ws.Cells(fila, 4).Value = wI
                ws.Cells(fila, 5).Value = cart(i, C_PRE)
                ws.Cells(fila, 6).Value = pA
                ws.Cells(fila, 7).Value = rY
                ws.Cells(fila, 8).Value = wI * rY
                totY = totY + wI * rY
                ' QTD: precio en cierre de trimestre; si entro despues, su ref YTD si es posterior
                pT = 0
                If dRefT.Exists(cod) Then pT = dRefT(cod)
                If pT <= 0 And dRefF(cod) >= fRefT Then pT = dRefA(cod)
                If pT > 0 Then
                    rQ = cart(i, C_PRE) / pT - 1
                    ws.Cells(fila, 9).Value = pT
                    ws.Cells(fila, 10).Value = rQ
                    ws.Cells(fila, 11).Value = wI * rQ
                    totQ = totQ + wI * rQ
                End If
                ws.Cells(fila, 12).Value = IIf(dRefF(cod) = fRefA, "Cierre anio", _
                                               "Desde " & Format(CDate(dRefF(cod)), "mm/yyyy"))
                fila = fila + 1
            End If
        End If
    Next i
    ...
    ws.Cells(fila, 3).Value = "Depositos a Plazo"
    ws.Cells(fila, 4).Value = pesoDep
    ws.Cells(fila, 8).Value = depY
    ws.Cells(fila, 11).Value = depQ
    ws.Cells(fila, 12).Value = "Carry aprox (tasa actual x dias)"
    totY = totY + depY: totQ = totQ + depQ
    fila = fila + 1
    If fila > 5 Then ws.Range("A4:L" & fila - 1).Sort Key1:=ws.Range("H4"), Order1:=xlDescending, Header:=xlNo
    ws.Cells(fila, 2).Value = "TOTAL"
    ws.Cells(fila, 2).Font.Bold = True
    ws.Cells(fila, 8).Value = totY
    ws.Cells(fila, 11).Value = totQ
    ws.Range(ws.Cells(fila, 8), ws.Cells(fila, 11)).Font.Bold = True
    FormatoInstitucional ws.Range("A3:L" & fila)
    ws.Range("D4:D" & fila).NumberFormat = "0.00%"
    ws.Range("E4:F" & fila).NumberFormat = "#,##0.0000"
    ws.Range("G4:H" & fila).NumberFormat = "0.0000%"
    ws.Range("I4:I" & fila).NumberFormat = "#,##0.0000"
    ws.Range("J4:K" & fila).NumberFormat = "0.0000%"
    ws.Columns("A:L").AutoFit
End Sub

Private Function VectorRefTrim(ByVal fObs As Date, ByRef dV As Object) As Long
    ' ultimo cierre de trimestre estrictamente anterior a fObs
    Dim objetivo As Date, arrV() As Long, j As Long
    Dim y As Integer, m As Integer
    y = Year(fObs): m = Month(fObs)
    Select Case m
        Case 1, 2, 3:   objetivo = DateSerial(y - 1, 12, 31)
        Case 4, 5, 6:   objetivo = DateSerial(y, 3, 31)
        Case 7, 8, 9:   objetivo = DateSerial(y, 6, 30)
        Case Else:      objetivo = DateSerial(y, 9, 30)
    End Select
    If dV.Count = 0 Then Exit Function
    arrV = ClavesOrdenadas(dV)
    For j = LBound(arrV) To UBound(arrV)
        If arrV(j) <= CLng(objetivo) Then VectorRefTrim = arrV(j)
        If arrV(j) > CLng(objetivo) Then Exit For
    Next j
End Function


'============================================================
' POSICIONAMIENTO ACTUAL VS BENCHMARK (v9.2)
' - Hoja "Posicionamiento": generada en cada corrida (ACTUAL desde cartera).
' - Hoja "Benchmark": la llena el equipo UNA vez (pesos target por concepto);
'   la macro la crea con la estructura si no existe y NUNCA la sobreescribe.
' Buckets bonos por dias al vcto: <1a, 1-3a, 3-5a, 5-7a, 7-10a, >10a.
' Soberano = rating "Gobierno Central". Depositos: 0-3m, 3-6m, 6-12m, >12m.
'============================================================
Private Function ConceptosPos() As Variant
    ConceptosPos = Array( _
        "Term Deposits", "  TD 0-3m", "  TD 3-6m", "  TD 6-12m", "  TD >12m", _
        "Commercial Papers", _
        "  Certificates of Deposit", "    CD 0-3m", "    CD 3-6m", "    CD 6-12m", "    CD >12m", _
        "  Commercial Papers", "    CP 0-3m", "    CP 3-6m", "    CP 6-12m", "    CP >12m", _
        "Sovereign Bonds", "  Sov <1y", "  Sov 1-3y", "  Sov 3-5y", "  Sov 5-7y", "  Sov 7-10y", "  Sov >10y", _
        "Corporate Bonds", "  Corp <1y", "  Corp 1-3y", "  Corp 3-5y", "  Corp 5-7y", "  Corp 7-10y", "  Corp >10y", _
        "Total")
End Function

Private Sub PrepararBenchmark()
    Dim ws As Worksheet, ex As Boolean, i As Long, c As Variant
    ex = False
    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets(SH_BMK)
    ex = Not ws Is Nothing
    On Error GoTo 0
    If ex Then Exit Sub                      ' ya existe: no tocar (pesos del equipo)
    Set ws = Hoja(SH_BMK)
    ws.Range("A1").Value = "F0 BENCHMARK - target weights (fill column B; the macro never overwrites this sheet)"
    ws.Range("A1").Font.Bold = True
    ws.Range("A2:B2").Value = Array("CONCEPT", "BMK WEIGHT")
    c = ConceptosPos()
    For i = LBound(c) To UBound(c)
        ws.Cells(3 + i, 1).Value = c(i)
    Next i
    ws.Range("B3:B" & 3 + UBound(c)).NumberFormat = "0.00%"
    FormatoInstitucional ws.Range("A2:B" & 3 + UBound(c))
    ws.Columns("A:B").AutoFit
End Sub

Private Function BucketBono(ByVal dias As Double) As String
    Select Case dias
        Case Is < 365:  BucketBono = "<1y"
        Case Is < 1095: BucketBono = "1-3y"
        Case Is < 1825: BucketBono = "3-5y"
        Case Is < 2555: BucketBono = "5-7y"
        Case Is < 3650: BucketBono = "7-10y"
        Case Else:      BucketBono = ">10y"
    End Select
End Function

Private Function BucketDep(ByVal dias As Double) As String
    Select Case dias
        Case Is <= 90:  BucketDep = "0-3m"
        Case Is <= 180: BucketDep = "3-6m"
        Case Is <= 365: BucketDep = "6-12m"
        Case Else:      BucketDep = ">12m"
    End Select
End Function

Private Sub EscribirPosicionamiento(ByVal fObs As Date, ByRef cart() As Variant, ByVal nD As Long)
    Dim ws As Worksheet, i As Long, fila As Long
    Dim d As Object, mtoTot As Double, k As String, esSob As Boolean
    Dim dBmk As Object, wsB As Worksheet, ufB As Long, c As Variant
    Dim act As Double, bmk As Variant
    ...
    For i = 1 To nD
        Select Case CStr(cart(i, C_CAT))
            Case "Papeles Comerciales"
                d("Commercial Papers") = Val(d("Commercial Papers")) + cart(i, C_MTO)
                ac = LCase$(CStr(cart(i, C_AC)))
                If InStr(ac, "cd") > 0 Or InStr(ac, "certificado") > 0 Then
                    d("  Certificates of Deposit") = Val(d("  Certificates of Deposit")) + cart(i, C_MTO)
                    k = "    CD " & BucketDep(Val(cart(i, C_DIAS)))
                Else
                    d("  Commercial Papers") = Val(d("  Commercial Papers")) + cart(i, C_MTO)
                    k = "    CP " & BucketDep(Val(cart(i, C_DIAS)))
                End If
                d(k) = Val(d(k)) + cart(i, C_MTO)
            Case "Depositos a Plazo"
                d("Term Deposits") = Val(d("Term Deposits")) + cart(i, C_MTO)
                k = "  TD " & BucketDep(Val(cart(i, C_DIAS)))
                d(k) = Val(d(k)) + cart(i, C_MTO)
            Case Else   ' Bonos
                esSob = (UCase$(Replace(CStr(cart(i, C_RAT)), " ", "")) = "GOBIERNOCENTRAL")
                If esSob Then
                    d("Sovereign Bonds") = Val(d("Sovereign Bonds")) + cart(i, C_MTO)
                    k = "  Sov " & BucketBono(Val(cart(i, C_DIAS)))
                Else
                    d("Corporate Bonds") = Val(d("Corporate Bonds")) + cart(i, C_MTO)
                    k = "  Corp " & BucketBono(Val(cart(i, C_DIAS)))
                End If
                d(k) = Val(d(k)) + cart(i, C_MTO)
        End Select
    Next i

    ' benchmark: leer pesos de la hoja (por concepto exacto)
    Set dBmk = CreateObject("Scripting.Dictionary")
    On Error Resume Next
    Set wsB = ThisWorkbook.Worksheets(SH_BMK)
    On Error GoTo 0
    If Not wsB Is Nothing Then
        ufB = UltFila(wsB)
        For i = 3 To ufB
            k = CStr(wsB.Cells(i, 1).Value)
            If Len(Trim$(k)) > 0 And IsNumeric(wsB.Cells(i, 2).Value) Then _
                dBmk(k) = CDbl(wsB.Cells(i, 2).Value)
        Next i
    End If

    Set ws = Hoja(SH_POS)
    ws.Cells.Clear
    ws.Range("A1").Value = "CURRENT POSITIONING AGAINST BENCHMARK — " & Format(fObs, "dd/mm/yyyy")
    ws.Range("A1").Font.Bold = True
    ws.Range("A2:D2").Value = Array("CONCEPT", "CURRENT", "BENCHMARK", "GAP")
    c = ConceptosPos()
    fila = 3
    For i = LBound(c) To UBound(c)
        k = CStr(c(i))
        ...
        Else
            act = Val(d(k)) / mtoTot
        End If
        ws.Cells(fila, 2).Value = act
        If dBmk.Exists(k) Then
            bmk = dBmk(k)
            ws.Cells(fila, 3).Value = bmk
            ws.Cells(fila, 4).Value = act - CDbl(bmk)
        End If
        If Left$(k, 2) <> "  " Then ws.Range(ws.Cells(fila, 1), ws.Cells(fila, 4)).Font.Bold = True
        fila = fila + 1
    Next i
    ws.Range("B3:D" & fila - 1).NumberFormat = "0.00%"
    FormatoInstitucional ws.Range("A2:D" & fila - 1)
    ws.Columns("A:D").AutoFit
End Sub


'============================================================
' TC (v9.5): lee el tipo de cambio USD/PEN de la hoja
' "Tipo de cambio" del FMS del dia. Busca una celda con
' USD/DOLAR y toma el primer numero razonable (1-10) a su
' derecha. Respaldo: Config B10 si no se encuentra.
'============================================================
Private Function LeerTC(ByVal wb As Workbook) As Double
    ' Hoja "T.Cambio" del FMS: A=fch_camb, B=cod_mone, C=sim_mone, D=com_vrtc
    ' USD = cod_mone 2 -> TC en columna D de esa fila.
    ...
    For Each hoja In wb.Worksheets
        If InStr(LCase$(hoja.Name), "cambio") > 0 Then Set ws = hoja: Exit For
    Next hoja
    If ws Is Nothing Then Exit Function

    ' 1) regla exacta: fila con cod_mone = 2 (col B) -> TC en col D
    For i = 2 To 60
        If Trim$(CStr(ws.Cells(i, 2).Value)) = "2" Then
            v = ws.Cells(i, 4).Value
            If IsNumeric(v) Then
                If v > 1 And v < 10 Then LeerTC = CDbl(v): Exit Function
            End If
        End If
    Next i

    ' 2) respaldo: fila con simbolo US$/USD -> primer numero 1-10 a la derecha
    For i = 1 To 60
        For j = 1 To 6
            s = UCase$(CStr(ws.Cells(i, j).Value))
            If s = "US$" Or InStr(s, "USD") > 0 Then
                For k = j + 1 To j + 3
...
'============================================================
' PROFORMA (v9.6): pesos objetivo de la cartera propuesta.
' El equipo la llena a mano; la macro la crea una vez y nunca
' la sobreescribe. Alimenta la columna PROFORMA del Summary.
'============================================================
Private Sub PrepararProforma()
    Dim ws As Worksheet, ex As Boolean
    ex = False
    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets("Proforma")
    ex = Not ws Is Nothing
    On Error GoTo 0
    If ex Then Exit Sub
    Set ws = Hoja("Proforma")
    ws.Range("A1").Value = "PROFORMA - target portfolio weights (fill column B; never overwritten by the macro)"
    ws.Range("A1").Font.Bold = True
    ws.Range("A2:B2").Value = Array("GROUP", "WEIGHT")
    ws.Range("A3").Value = "Deposits"
    ws.Range("A4").Value = "Short Term Debt Instruments"
    ws.Range("A5").Value = "Sovereign Bonds"
    ws.Range("A6").Value = "Corporate Bonds"
    ws.Range("A7").Value = "Short Term Fixed Income Funds"
    ws.Range("B3:B7").NumberFormat = "0.00%"
    FormatoInstitucional ws.Range("A2:B7")
    ws.Columns("A:B").AutoFit
End Sub


'============================================================
' VALOR CUOTA (v9.9): copia la serie del archivo del equipo
' (Config B11 = ruta; hoja PROFUTURO: A=fecha, B=PRF0) a la
' hoja "ValorCuota" de este libro. Si el archivo no esta
' disponible, conserva la copia anterior y avisa.
'============================================================
Private Function ActualizarValorCuota() As String
    Dim ruta As String, wb As Workbook, wsOri As Worksheet, wsDst As Worksheet
    Dim uf As Long, i As Long, n As Long
    Dim datos() As Variant

    ruta = Trim$(CStr(Hoja(SH_CFG).Range("B11").Value))
    If Len(ruta) = 0 Then
        ActualizarValorCuota = "Valor cuota: sin ruta en Config B11 (se omite)."
        Exit Function
    End If

    On Error GoTo Falla
    Application.StatusBar = "Actualizando valor cuota..."
    Set wb = Workbooks.Open(ruta, ReadOnly:=True, UpdateLinks:=0)
    Set wsOri = wb.Worksheets("PROFUTURO")
    uf = wsOri.Cells(wsOri.Rows.Count, 1).End(xlUp).Row
    If uf < 3 Then GoTo Falla

    ReDim datos(1 To uf - 2, 1 To 2)
    n = 0
    For i = 3 To uf
        If IsDate(wsOri.Cells(i, 1).Value) Or IsNumeric(wsOri.Cells(i, 1).Value) Then
            If IsNumeric(wsOri.Cells(i, 2).Value) Then
                If wsOri.Cells(i, 2).Value > 0 Then
                    n = n + 1
                    datos(n, 1) = wsOri.Cells(i, 1).Value
                    datos(n, 2) = wsOri.Cells(i, 2).Value
                End If
            End If
        End If
    Next i
    wb.Close SaveChanges:=False: Set wb = Nothing
    If n = 0 Then GoTo Falla

    Set wsDst = Hoja("ValorCuota")
    wsDst.Cells.Clear
    wsDst.Range("A1:B1").Value = Array("FECHA", "VC F0")
    FormatoInstitucional wsDst.Range("A1:B1")
    wsDst.Range("A2").Resize(n, 2).Value = datos
    wsDst.Range("A2:A" & n + 1).NumberFormat = "dd/mm/yyyy"
    wsDst.Range("B2:B" & n + 1).NumberFormat = "#,##0.0000000"
    Application.StatusBar = False
    ActualizarValorCuota = "Valor cuota: " & n & " observaciones actualizadas."
    Exit Function
...
Falla:
    ...
    If Not wb Is Nothing Then wb.Close SaveChanges:=False
    Application.StatusBar = False
    ActualizarValorCuota = "Valor cuota: NO se pudo leer el archivo (se conserva la copia anterior)."
End Function
